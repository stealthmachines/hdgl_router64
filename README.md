...
## phi-bridge: hooking into a traditional router OS

### How it works

The `boot` command is the hook mechanism. Instead of staying in HDGL forever,
you inspect the hardware, then hand off to the router's native OS while leaving
a snapshot of the HDGL state in physical memory where the OS can read it.

```
Router64> boot
[phi-bridge] mailbox written at 0x50000
[phi-bridge] chainloading router OS...
```

What happens:

1. HDGL writes a 104-byte **phi-bridge mailbox** at physical address `0x50000`.
   The mailbox contains the phi-lattice consensus state, wave aggregate, NIC
   count, UART count, CPU CPUID, and memory map from the HDGL boot — a snapshot
   of the hardware as HDGL observed it.

2. HDGL copies a small 32-bit trampoline (180 bytes) to `0x4000`.

3. HDGL drops from 64-bit long mode back to 16-bit real mode via the trampoline.

4. The trampoline calls INT 13h to load the router OS MBR (from drive 0x81 by
   default — the second hard disk, assuming HDGL is on USB or drive 0x80) into
   `0x7C00`, then far-jumps there.

5. The router OS boots normally — GRUB, FreeBSD bootloader, whatever.

6. The **mailbox at `0x50000` persists**. Linux, FreeBSD, OpenBSD, and virtually
   all x86 OSes do not touch conventional memory below 640KB that was not
   explicitly marked available in the E820 map. `0x50000` sits in this region.

### Reading the mailbox from the router OS

Use `hdgl-read.c` (included in `src/`):

```sh
# Build
gcc -O2 -o hdgl-read src/hdgl-read.c

# Run (requires root)
sudo ./hdgl-read
```

Output on OpenWrt/Linux after booting via `boot`:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  HDGL phi-bridge mailbox @ 0x50000
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  version    : 1
  hdgl_drive : 0x80  (HDGL booted from this drive)
  cpu        : family=6 model=60 step=4 (0x000306C4)
  memory     : 523775 KB (0.5 GB)
  nics       : 3 detected at HDGL boot
  uarts      : 1 detected at HDGL boot
  consensus  : LOCK
  wave_agg   : 0x0000000080C0C0E8
  phi_tick   : 0x101010 (ptr; map /dev/mem to read live)
  omega_base : 0x200000
  lat_base   : 0x101020
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```sh
sudo ./hdgl-read --json | jq .
```

### Targeting a specific drive

```
Router64> boot        # chainload 0x81 (second disk, default)
Router64> boot 128    # chainload 0x80 (decimal; same as 0x81 if that's drive 1)
Router64> boot 129    # chainload 0x81 explicitly
```

Drive numbers follow BIOS convention: `0x80` = first hard disk, `0x81` = second,
`0x82` = third. USB sticks and CF cards get enumerated as hard disks by most
BIOSes. If HDGL is on USB (0x80) and the router OS is on mSATA (0x81), the
default `boot` with no argument chainloads correctly.

### Mailbox structure (for custom readers)

```c
/* At physical address 0x50000, 104 bytes */
struct hdgl_mailbox {
    uint32_t magic;         /* 0x48444C47 'HDLG' — verify before reading */
    uint32_t version;       /* 1 */
    uint64_t phi_tick_ptr;  /* physical addr of live phi-tick counter */
    uint64_t omega_base;    /* 0x200000 — Omega graph */
    uint64_t nic_table;     /* 0x104000 — NIC table */
    uint64_t nic_count;     /* NICs detected */
    uint64_t uart_table;    /* 0x104200 — UART table */
    uint64_t uart_count;    /* UARTs detected */
    uint64_t lat_base;      /* 0x101020 — phi-lattice */
    uint64_t consensus;     /* 1 = LOCK at chainload time */
    uint64_t wave_agg;      /* Dn(r) aggregate, healthy = 0x80C0C0E8 */
    uint64_t cpu_cpuid;     /* CPUID leaf1 eax */
    uint64_t mem_kb;        /* usable RAM in KB */
    uint32_t hdgl_drive;    /* BIOS drive HDGL booted from */
    uint32_t reserved;
};
```
...
fields = struct.unpack_from('<II8Q2IQ', raw)  # approximate
magic = fields[0]
if magic != MAGIC:
    raise RuntimeError(f'No HDGL mailbox (found 0x{magic:08X})')

# Or use ctypes for the full struct
```

Read from FreeBSD (pfSense/OPNsense):

```sh
# /dev/mem works the same way; you may need to set kern.securelevel=-1
sysctl kern.securelevel=-1   # if restricted
sudo ./hdgl-read
```
...
```sh
# OpenBSD restricts /dev/mem; use amd64-specific iopl or compile with pledge removed
# Simplest: read the physical page directly using dd
sudo dd if=/dev/mem bs=1 skip=$((0x50000)) count=104 2>/dev/null | od -An -tx4 | head -7
```

### What this enables

The router's native OS gets a **hardware genome** from HDGL without having to
run any discovery itself. The NIC count, UART list, CPU identity, memory map,
and phi-lattice consensus state are all pre-computed by HDGL's bare-metal
observers and handed off via the mailbox.

Conversely, if the router OS is OpenWrt or pfSense and you have shell access,
`hdgl-read` lets you verify that the hardware HDGL observed matches what the
OS sees — a cross-check between the bare-metal view and the OS view. A
mismatch in NIC count between HDGL's PCI scan and the OS's device enumeration
points to a driver, firmware, or hardware issue.

For automation: `hdgl-read --json` integrates with monitoring systems,
configuration management tools, or simply `jq` pipelines.

### Limitations

The mailbox is a one-time snapshot written at chainload time. If the router OS
reclaims `0x50000` (unusual but possible if the OS uses a non-standard memory
allocator or is 32-bit), the mailbox will be overwritten. In practice this does
not happen on any of the tested hardware profiles because the E820 map that HDGL
built marks that region as BIOS-reserved.

The phi-tick counter at `0x101010` is a live value if the HDGL runtime is still
in memory (which it is, since the OS doesn't touch `0x8000–0xFFFF`), but HDGL
is not ticking any more once the chainload happened — the phi-tick counter froze
at the moment the trampoline ran. It is a historical value, not a live one.

If you want a continuously ticking phi-lattice co-running with the router OS,
that would require a different architecture: a small resident installed in
conventional memory that the OS could trigger via a software interrupt, or a
separate microcontroller communicating over the second serial port.
