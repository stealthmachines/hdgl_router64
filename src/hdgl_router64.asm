...
.sh_help_text   db \
    ...
    'help       this list',13,10,'nic        list detected router NICs',13,10,'uart       list detected serial ports',13,10,0
...
; ── GDTs (must be accessible from 16-bit real mode) ──────────────────────
[BITS 16]
align 8
; 32-bit transitional GDT
.gdt32_start:
...
; 64-bit GDT (L=1 in code descriptor)
.gdt64_start:
    dq 0                           ; null
    dq 0x00AF9A000000FFFF          ; 64-bit code: L=1, P=1, DPL=0, C/D=1
    dq 0x00CF92000000FFFF          ; 64-bit data (same as 32-bit)
.gdt64_end:
.gdt64_ptr:
    dw .gdt64_end - .gdt64_start - 1
    dq .gdt64_start                ; 64-bit pointer
